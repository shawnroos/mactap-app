...
`midi-bridge/m4l/MacTap.amxd` is a ready Max MIDI Effect. Generated by
`build_device.py`; edit that, not the JSON.

1. Run the bridge with OSC: `./build/mactap-midi --osc 127.0.0.1:7400`
2. Drag `MacTap.amxd` onto a MIDI track in Live, then a Drum Rack after it.
3. Set the track's MIDI From to **No Input** — the device is the input;
   MacTap's MIDI port on top would double every knock.
4. Knock.

The device shows Sensitivity, Floor, Ceiling, Curve, Gate, Note L/R
and an L/R toggle. Every control sends its value to the bridge as you
turn it and again when the set loads, so the bridge always matches
what the set saved. Two buttons flash left/right and a box shows the
last velocity.

If Live refuses the file, the same patch is in `MacTap.maxpat`: add an
empty *Max MIDI Effect*, click its edit button, select all and delete,
open `MacTap.maxpat` in a text editor, copy all, paste into the
patcher, then save.
