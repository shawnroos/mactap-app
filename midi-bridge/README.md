...
One folder is the whole thing: the device, a launcher script, and the
bridge binary. The device starts the bridge when it loads and stops it
when it unloads — no terminal.

```
./midi-bridge/install.sh
```

That builds everything and puts it in Live's User Library. Then in Live:

1. Browser › User Library › Presets › MIDI Effects › Max MIDI Effect ›
   MacTap › drag **MacTap.amxd** onto a MIDI track. Add a Drum Rack after it.
2. Set the track's MIDI From to **No Input** — the device is the input.
3. Knock. The status line at the bottom reads `running — knock`.

The panel has Sensitivity, Floor, Ceiling, Curve, Gate, Note L/R, an L/R
toggle, two buttons that flash per side, a last-velocity box, a status
line and a Restart button. Every control sends its value to the bridge
as you turn it and again when the set loads, so a saved set restores the
bridge. All are Live parameters — automatable and mappable.

One device per set: two would both bind UDP 7400, and the second bridge
refuses to start anyway.

How it works: `[node.script mactap-launch.js]` (Node for Max ships with
Live) spawns `mactap-midi --osc` from the device's folder with a pipe on
its stdin; the bridge exits the moment that pipe closes, so nothing is
left running when the device goes. `build_device.py` writes the patch as
JSON and wraps it in the `.amxd` container read off Live's own template.

If Live refuses the file, the same patch is in `MacTap.maxpat`: add an
empty *Max MIDI Effect*, click edit, select all and delete, copy the
file's text, paste into the patcher, save.
